...
interface RoleStateEditModalProps {
  role: Role;
  sessionId: string;
  scopeId: string;
  onClose: () => void;
...
type AttrRow = { key: string; value: number };
type MeterRow = { name: string; value: number; max: number };
...
function parseAttrs(attrs: Role["attributes"]): AttrRow[] {
...
function parseMeters(meters: Role["meters"]): MeterRow[] {
...
function spotsToText(spots: string[]): string {
  return spots.join(", ");
}

function textToSpots(text: string): string[] {
  return text
    .split(/[,，]/)
    .map((s) => s.trim())
    .filter(Boolean);
