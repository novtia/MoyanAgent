...
fn resolve_list_dir(conn: &db::DbConn, session_id: &str, path: Option<&str>) -> AppResult<PathBuf> {
...
fn list_dir_entries(dir: &Path) -> AppResult<Vec<ProjectDirEntry>> {
    ...
    for entry in fs::read_dir(dir)
        .map_err(|e| AppError::Other(format!("list_project_dir: read_dir {:?}: {e}", dir)))?
    {
...
fn resolve_validated_path(conn: &db::DbConn, session_id: &str, path: &str) -> AppResult<PathBuf> {
    ...
    if path.is_dir() {
        fs::remove_dir_all(path).map_err(|e| {
            AppError::Other(format!(
                "delete_project_path: remove_dir_all {:?}: {e}",
                path
            ))
        })?;
    } else {
...
fn copy_path_recursive(from: &Path, to: &Path) -> AppResult<()> {
    if from.is_dir() {
        fs::create_dir_all(to)
            .map_err(|e| AppError::Other(format!("copy_project_path: mkdir {:?}: {e}", to)))?;
        for entry in fs::read_dir(from)
            .map_err(|e| AppError::Other(format!("copy_project_path: read_dir {:?}: {e}", from)))?
        {
            let entry =
...
pub fn create_project_dir(
    ...
    fs::create_dir_all(&file_path)
        .map_err(|e| AppError::Other(format!("create_project_dir: mkdir {:?}: {e}", file_path)))?;
    Ok(())
}
...
pub fn create_project_file(
    ...
    fs::write(&resolved, bytes.as_bytes())
        .map_err(|e| AppError::Other(format!("create_project_file: write {:?}: {e}", resolved)))?;
    Ok(())
}
...
pub fn import_external_path_to_project(
    ...
    copy_path_recursive(&src, &dest).map_err(|e| match e {
        AppError::Other(msg) => {
            AppError::Other(msg.replace("copy_project_path:", "import_external_path_to_project:"))
        }
        other => other,
    })
}
...
pub fn write_project_file_bytes(
    ...
    if let Some(parent) = resolved.parent() {
        if !parent.as_os_str().is_empty() {
            fs::create_dir_all(parent).map_err(|e| {
                AppError::Other(format!("write_project_file_bytes: mkdir {:?}: {e}", parent))
            })?;
        }
    }
